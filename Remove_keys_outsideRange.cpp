...
void inorder_range(struct node *btree,int key1,int key2)
{
	if(btree)
	{
		inorder_range(btree->left,key1,key2);
		if(btree->data < key1 ||  btree->data > key2)
		{
			cout << "removing the data: "<< btree->data << "...";
			struct node *_remove = findelement(btree->data);
			Delete(&btree,_remove);
		}	
		inorder_range(btree->right,key1,key2);
	}
}
void remove_in_range(struct node *btree,int key1,int key2)
{
	if(!btree) return;
	struct node * traversal= btree;
	if(traversal->data < key1)
		traversal =traversal->right;
	else if( traversal->data >key2)
		traversal = traversal->left;
	inorder_range(traversal,key1,key2);
}
